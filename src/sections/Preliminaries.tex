...
\begin{align*}
  ...
  \mathcal{X}_{\textit{end}} &= \set{ \vect{x} \in \modelconfigurationspace{} \mid g_i(\vect{x}) \leq
...
\begin{algorithm}[p]
  ...
  \(\textrm{cost}_{\textit{prev}} = \infty\)\; converged = false \; \While{\(!\; \textrm{converged}\)}{
    ...
    \If{\(\frac{\textrm{cost}_{\textit{prev}} -
        \textrm{cost}}{\textrm{cost}_{\textit{prev}}} < \epsilon \) } {
      converged = true
    }\;
    \(\textrm{cost}_{\textit{prev}} = \textrm{cost}\)\;
  }\;
\end{algorithm}
...
\begin{align}
  ...
  \text{s.t.} \; &\rho_2(0) - V_2( 0,\bar{\vect{x}}) - L(\bar{\vect{x}})
                  \bigl( \rho_1(T_1) - V_1(T_1,\bar{\vect{x}}) \bigr) \; . \nonumber
\end{align}
