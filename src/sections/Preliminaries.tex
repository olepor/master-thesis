...
the control input. Assume that an open loop nominal trajectory \(\vect{x}_0 \colon
...
\begin{align}
  ...
  \bar{\vect{x}}(t) &= \big( \vect{x} - \vect{x}_0 \big)(t) \\
  \bar{\vect{u}}(t) &= \big(\vect{u} - \vect{u}_0 \big)(t).
\end{align}
Then, transforming \cref{eq:dynamicalsystem} to the new coordinate frame one
obtains
\begin{equation}
  \label{eq:dynamicalsystem-coordinatechange}
  \dot{\bar{\vect{x}}} = \dot{\vect{x}} - \dot{\vect{x}}_0 = f\big( \vect{x}_0(t) + \bar{\vect{x}}(t), \vect{u}_0(t) + \bar{\vect{u}}(t) \big) - \dot{\vect{x}}_0(t)
\end{equation}
...
\begin{equation}
  ...
  \dot{\bar{\vect{x}}} = f_{cl}\big( t,\bar{\vect{x}}(t) \big).
\end{equation}
...
\begin{definition}
  ...
  = f_{cl}\big( t,\vect{x}(t) \big) \) is a map \(F \colon [0,T] \rightarrow
...
a finite sum
\begin{equation}
  \int_{0}^{T} \vol \big( F(t) \big) \, \mathrm{d}t \rightarrow \sum_{k=1}^{N} \vol \big( F(t_{k}) \big) \label{eq:discrete-costfunction}.
\end{equation}
...
{ % Mini environment scope
  ...
  \begin{mini*}[2]
    ...
    \addConstraint { \begin{aligned}
      ...
      \bigr) \\  &
      - L_t (t,\vect{x}) \bigl(  {t \p*{T - t}} \bigr)
    \end{aligned}}%
    {\text{ is SOS}}
    ...
    {\quad \forall k \in \set{1, \ldots, N}} \addConstraint {L_t (t,
      \vect{x}),\, L_{0,i}(\vect{x}),\, L_{\mathcal{E},k}(\vect{x})}%
    {\text{ are SOS}}%
