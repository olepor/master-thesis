...
the sampling rate~\cite{tobenkinInvariantFunnelsTrajectories2010}. If \(t_{k}
\in [0,T]\), where \(k \in \set{1,\ldots,N}\), the optimization equations become
\begin{mini}
  ...
  \addConstraint{\dot{\rho}(t_{k}) - \dot{V}_{k}(\bar{x}) - L_{k}(\bar{x})
    ...
    \set{1,\ldots,N}}
  \addConstraint{\rho(t_{1}) - V_{1}(\bar{x}) -
  ...
  \addConstraint{1 - {\bar{x}}^{T} S_{k} \bar{x} - L_{\mathcal{E,k}} \left(
    ...
    \set{1,\ldots,N}} %
  \addConstraint{S_{k} \succeq 0} {} {\forall k \in
    \set{1,\ldots,N}} %
