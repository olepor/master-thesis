...
in figure~\ref{fig:reachable-set-dubin}.
...
y-axis looks like figure~\ref{fig:monte-carlo-sim}.
...
in section~\ref{sec:Funnels}. Given a trajectory, the goal is to compute a robust
...
\begin{align}
  ...
  \bar{u}(t) &= (u - u_0)(t).
\end{align}
Then, changing~\eqref{eq:dynamicalsystem} to these new coordinates one obtaines
\begin{equation}
...
the system~\eqref{eq:dynamicalsystem} needs to be polynomial, and parameterized by
...
the system~\eqref{eq:dynamicclosedloop} around the nominal trajectory \(x_0\)
...
\(F(t)\) is the set of states the system~\eqref{eq:dynamicclosedloop} can be in
at time \(t\), then
\begin{equation}
...
\begin{definition}
  ...
  condition~\eqref{eq:reachableset}.
\end{definition}
...
sufficient condition for containing the reachable~\eqref{eq:reachableset} set in
...
that satisfies~\eqref{eq:funnelsufficient}, and is a valid funnel in the sense of
...
The optimization problem in~\eqref{eq:funneloptimizationproblem} would prove
...
Then rewriting~\eqref{eq:funneloptimizationproblem}~as in terms of positivity
...
function~\eqref{eq:discrete-costfunction} is introduced as
\begin{equation}
...
constraints~\eqref{eq:sufficient-conditions}-\eqref{eq:sufficient-conditions3},
...
{ % Mini environment scope
  ...
  \begin{mini*}[1]
    ...
    {\text{ is SOS}} \addConstraint {\rho(0) - V(0, \x) - \sum_i^N L_{0, i}(\x)
      g_{0, i}(\x)}%
    {\text{ is SOS}} \addConstraint { 1 - \x^T S_k \x - L_{\E, k}(\x)
      \sqb*{\rho(t_k) - V(t_k, \x)} }%
