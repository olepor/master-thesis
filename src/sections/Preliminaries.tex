...
If the discrete state space model is expanded to include \(\vect{w}\) as the
...
\begin{align}
  ...
  \bar{\vect{u}}(t) &= (\vect{u}in - \vect{u}_0)(t).
\end{align}
...
\begin{figure}
  \centering \includegraphics{figures/experiments/lyapunov_visualization}
  ...
  \begin{mini*}[1]
    { \substack { V, \rho, L, L_t,
      ...
      \sum_{k = 1}^{N} \vol \p{\set{\vect{x} \mid \vect{x}^T \matr{S}_k \vect{x}
          \le 1}} } {} {} \addConstraint { \dot{\rho}(t) - \dot{V}(t,\vect{x}) -
      L(t,\vect{x}) \sqb*{ V(t,\vect{x}) - \rho(t) } - L_t (t,\vect{x}) \sqb*{t
        \p*{T - t}} }%
...
\begin{mini}
  ...
  \addConstraint{L_{0,i}(\bar{\vect{x}}), L_{\mathcal{E},k}} {\, \text{are SOS}}
