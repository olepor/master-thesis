...
\ac{RRT} robust motion planner based on \textcite{Lav06}.
Using robust motion primitives has several advantages. Firstly, they are robust
to uncertainty, and thus, as long as the uncertainties in the system are akin to
the assumptions put on the incoming uncertainty parameters, the dynamical system
will not leave the funnel, and hence if the funnel is not in collision, neither
will the system be. Secondly, with the assumption that the motion primitives are
...
primitive that is close to an obstacle, as one that is far away, since the
funnel is guaranteed to be collision-free in both cases. This means that a
robust motion primitive algorithm can perform more aggressive maneuvers than one
that is inherently conservative about its environment and
...
seeks to optimize the cost function. Following in \cref{sec:adding-uncertainty}
...
motion planning literature~\cite{Betts_1998}. Thus the initial trajectories can
be generated many different methods, however, the \textit{direct collocation
...
\end{equation}
where \(\matr{R} = 1\)~\cite{majumdarRobustOnlineMotion2013}. This is because it
will minimize the input, and give a smooth output trajectory.
...
\begin{figure}
  \begin{minipage}[c]{0.3\textwidth}
    ...
    width=.85\textwidth]{figures/method/left-trajector}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{0.3\textwidth}
    \includegraphics[trim={5cm 5cm 5cm 5cm},
...
\cref{eq:system-error-dynamics}) gives
\begin{equation}
...
dynamical model \eqref{eq:model-dynamics} thesis the size of the funnel
...
\]
with the projected ellipsoid given by
\[
...
\]
and \(P\) is an \(n\times n_p\) projection matrix.
...
the size of the funnels will in general be larger than if it is given a better
controller. Thus in order to further minimize the size of the funnel in the
xy-plane, this method, along with the cost function from
\cref{subsec:xy-cost-function} is used to make traversal through obstacles in
the world space easier. The initial controller fed to the algorithm can be
optimized with the goal of minimizing the size of the funnel, given a few
conditions on the system~\cite[sec~4.3.2]{majumdarFunnelLibrariesRealtime2017}.
...
\end{equation}
The dynamical model in \cref{eq:model-dynamics} is control affine, since
\begin{equation}
  \dot{\vect{x}} = %
...
(\bar{\vect{u}}_f,\rho,L_t,L_{0,i},S_k) \) while keeping \( (V,L,L_{\epsilon,k})
...
the system \(\hat{\mathcal{F}} = \set{F_n \in \R^n \mid \Psi_{c,i}(F_i), F_i \in
  \mathcal{F}}\), where \(\mathcal{F}\) is the basic set of funnels, the
transformation is already known, and the composability is therefore straight
forward to compute.

\begin{figure}
  \centering {\includegraphics[width=.8\textwidth]{figures/method/funnel-composition}}
...
example of which can be seen in \cref{fig:funnel-simulated-overlaid}, and
further explored for a single funnel in
\cref{fig:outer-approximation-monte-carlo}.
...
proper outer approximations of the time-reachable sets for the dynamical system.
 Therefore, the
conclusion is that the uncertain trajectories are contained within the funnels
used in the planner, and the trajectories can be seen as robust to uncertainty
given the uncertainty and dynamical assumptions made.
...
airplane is to the final configuration, and is defined as~\cite{kuffnerEffectiveSamplingDistance2004}
\[
  \rho(\vect{x}_{1}, \vect{x}_{2}) = w_{1}\norm{\mathnormal{X_{1}} -
    \mathnormal{X_{2}}} + w_{2}f(\theta_1,\theta_2) ,
\]
where \(\norm{\mathnormal{X_{1}} - \mathnormal{X_{2}}}\) is the standard
Euclidean metric, \(f\) is a function giving the distance between headings. The
rotations and distance is then scaled relative to the translation distance.
...
\begin{figure}
  ...
  \caption[Distance metrics for non-holonomic vehicles]{Consider two poses
    ...
    distance it is harder to get to due to differential constraints. In this
    paper, we propose a directed distance function applicable to unicycle type
    models, that properly reflects the true cost-to-go of the system under the
    non-holonomic constraint \textcite{parkFeedbackMotionPlanning2015}.}
...
\begin{algorithm}
  ...
  \(\mathcal{G}.\mathnormal{init}(q_0)\) \For{\(i \leftarrow 1\) \KwTo \(k\)}{ \(q_{\mathit{rand}}
    \leftarrow \) \RandConf{} \; \(q_{\mathit{near}} \leftarrow \)
    \NearestVertex{\(q_{\mathit{rand}}, \mathcal{G}\)}\; \(q_{\mathit{new}} \leftarrow \)
    \Extend{\(q_{\mathit{near}}, q_{\mathit{rand}} \)}\; \If{\(q_{\mathit{new}} \in
      \modelconfigurationspacefree{} \) } {
      \(\mathcal{G}\).\AddVertex{\(q_{\mathit{new}}\)}\;
      \(\mathcal{G}\).\AddEdge{\(q_{\mathit{near}}, q_{\mathit{new}}\)} \; \If{\(q_{\mathit{new}} \in
        \mathcal{X}_{\mathit{goal}}\)}{ return \ExtractBranch{\(\mathcal{G}\)} \; } } }
\end{algorithm}
