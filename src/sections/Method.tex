...
also called a reachable set~\cref{subsec:reachable-set}. But in order to verify
...
have been generated through any number of methods, however, the \textit{direct
  collocation method} was chosen as it builds locally optimal trajectories from
a discrete set of sampled points along a sought trajectory, which is beneficial
for the discrete funnel verification in~\cref{subsec:generating-funnels}.
...
algorithm~\cref{alg:funnelalgorithm}, and is implemented in software through the
~\cite[sostools]{sostools} toolbox.

However, two obstacles remain, as the dynamics for the
model~\cref{eq:model-dynamics} are still not polynomial, and the \ac{SOS} method
...
mentioned in~\cref{subsec:generating-funnels}. In the forest traversal case the
...
Since minimizing the volume of the ellipsoid \(\pi(\mathcal{E})\) using an
...
in~\cref{sec:adding-uncertainty}, is what forms the basis of the funnel
computations as robust motion primitives for traversal in a dense forest
environment, as summarized in algorithm~\cref{alg:funnelalgorithm-extended}.
...
condition~\cref{eq:funnelsufficient}
...
cyclic coordinates. Hence the definition used
...
composable in the sense of definition~\cref{def:funnel-composition}, they are
...
\cref{alg:create-funnel-graph}, where a funnel is a vertice in the graph, and an
