...
\rrtfunnel{} will in section(blablal) will develop. section(bla) is lsadkjfa;sldkjf.
...
%   \KwIn{\(\mathcal{F}\) -- The basis set of funnels computed around the
%   nominal trajectories.} \KwOut{\(\mathcal{G}(\mathcal{F})\) -- Directed graph
...
%   \If{\(F_{i}(t_{k}) \subset F_{j}(t_{l})\)} { \(\mathcal{G} \leftarrow{}
%   \left( F_{i}(t_{k}), F_{j}(t_{l}) \right)\) } } } } }
