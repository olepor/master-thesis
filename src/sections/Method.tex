...
to verify the robust regions surrounding a trajector, first the trajectories
...
\end{equation}
an intial condition is obtained. In general however, any semi-algebraic set will
do, and the algorithm is not constrained to this one initial condition set in
...
In the \rrtfunnel{} algorithm, the size of the of the hyperellipsoid which
...
input \(u\). Having a large \(\theta\) semi-axis in the hyperellipsoid can also
...
the cyclic coordinates of the system withouth changing the solution to the
