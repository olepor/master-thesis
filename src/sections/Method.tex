...
can be written as \(\dot{\bar{\vect{x}}} = f_{cl}(t,\vect{x})\) thanks to the controller,
...
\(f_{cl}(t,\vect{x})\) by direct substitution of \(\vect{u}\).
...
is \(\mathcal{E} = \set{\bar{\vect{x}} \in \R^n | \bar{\vect{x}}^TS_{k}\bar{\vect{x}} \leq 1}\)
with
\[
...
polynomial \(u_f(t,\bar{\vect{x}})\) to the set of decision variables in the original
...
cyclic- and non-cyclic coordinates of the form \(\vect{x} = [x_c\, x_{nc}]\), the
state dynamics \(\dot{\vect{x}} = f(\vect{x} (t), u(t))\) only depends on the non-cyclic
coordinates of the system. Thus, a trajectory of the form \(t \rightarrow
(x(t),u(t))\) which solves \(\dot{\vect{x}} = f(\vect{x}(t),u(t))\) can then be transformed
...
\[
  ...
  \end{cases}.
\]
However, since \(\mathnormal{dim}(\vect{x}) = \mathnormal{dim}(\vect{x}_c) \), for
...
\[
  (x,y,\theta) = (\mathnormal{X}, \mathnormal{Y},
  \mathnormal{\Theta})
\]
...
along the trajectories they verify. These regions of invariance are then
given as the motion primitives to the global \ac{RRT} motion planner, which uses
these funnels to compose a tree of motion primitives through the configuration
space, and hence safe traversal can be made, given that the funnels abide by the
